...
const Avatar = (props: AvatarProps) => {
  ...
  const getStatusClasses = (): string => {
    ...
    const baseStatusClasses =
      "absolute bottom-0 right-0 transform translate-x-1/4 translate-y-1/4 border-2 border-white rounded-full";
  ...
  // Get the size in pixels for the Image component
  ...
  return (
    <div className="relative inline-flex" data-testid={testId}>
      {src ? (
        <div
          className={`${getSizeClasses()} rounded-full overflow-hidden ${className}`}
        >
          <Image
            src={src}
            alt={alt}
