...
describe('ApproveRejectUserUseCase', () => {
  ...
  const approveWithRoleData = { approvalStatus: "Approved" as const, roleIds: 2 };
  ...
  it('should handle errors when approving or rejecting a user', async () => {
    ...
    userRepositoryStub.approveRejectUser = async (id: number, data: { approvalStatus: "Approved" | "Rejected"; roleIds?: number }) => {
    ...
    // Act and Assert
    await expect(approveRejectUserUseCase.execute(userId, approveData)).rejects.toThrow(errorMessage);
  ...
  it('should pass the role ID when approving with a role', async () => {
    ...
    // Act
    const result = await approveRejectUserUseCase.execute(userId, approveWithRoleData);
