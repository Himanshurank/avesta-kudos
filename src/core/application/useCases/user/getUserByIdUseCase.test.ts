...
describe('GetUserByIdUseCase', () => {
  ...
  it('should handle different user IDs', async () => {
    ...
    // Verify the repository was called with correct parameters
    expect(userRepositoryStub.getUserById).toHaveBeenCalledWith(differentUserId);
  });
  ...
  it('should handle user not found error', async () => {
    ...
    // Act and Assert
    await expect(getUserByIdUseCase.execute(nonExistentUserId)).rejects.toThrow(errorMessage);
    
    // Verify the repository was called with correct parameters
    expect(userRepositoryStub.getUserById).toHaveBeenCalledWith(nonExistentUserId);
  });
}); 