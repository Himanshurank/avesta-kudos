...
describe('GetAllUsersUseCase', () => {
  ...
  // Test data
  const mockUsers = [
    new User(1, 'user1@example.com', 'User One', [{ id: 1, name: 'USER' }], 'Approved', new Date(), new Date()),
    new User(2, 'user2@example.com', 'User Two', [{ id: 1, name: 'USER' }], 'Pending', new Date(), new Date()),
    new User(3, 'admin@example.com', 'Admin User', [{ id: 2, name: 'ADMIN' }], 'Approved', new Date(), new Date())
  ];
  ...
  it('should get users with pagination parameters', async () => {
    ...
    // Verify the repository was called with correct parameters
    expect(userRepositoryStub.getAllUsers).toHaveBeenCalledWith(paginationParams);
  });
