...
describe('UpdateUserUseCase', () => {
  ...
  it('should update a user successfully with name change', async () => {
    ...
    expect(userRepositoryStub.updateUser).toHaveBeenCalledWith(userId, updateData);
  });
  ...
  it('should handle user not found error', async () => {
    ...
    // Act and Assert
    await expect(updateUserUseCase.execute(nonExistentUserId, updateData)).rejects.toThrow(errorMessage);
    
    // Verify the repository was called with correct parameters
    expect(userRepositoryStub.updateUser).toHaveBeenCalledWith(nonExistentUserId, updateData);
  });
  ...
  it('should handle validation errors', async () => {
    ...
    // Act and Assert
    await expect(updateUserUseCase.execute(userId, invalidUpdateData)).rejects.toThrow(errorMessage);
    
    // Verify the repository was called with correct parameters
    expect(userRepositoryStub.updateUser).toHaveBeenCalledWith(userId, invalidUpdateData);
  });
}); 